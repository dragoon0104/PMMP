...
class Server{
	...
	public function getOfflinePlayerData($name){
		...
		if(\file_exists($path . "$name.dat")){
			...
			$this->logger->notice($name . "のプレイヤーデータを作成します");
		}
	...
	public function generateLevel($name, $seed = \null, $generator = \null, $options = []){
		...
		if($generator !== \null and \class_exists($generator) and \is_subclass_of($generator, Generator::class)){
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath){
		...
		if(($memory = \str_replace("B", "", \strtoupper($this->getConfigString("memory-limit", "256M")))) !== \false){
			...
			if($real < 128){
				$this->logger->warning($this->getName() . " may not work right with less than 128MB of RAM", \true, \true, 0);
			}
		...
		$this->logger->info("This server is running " . $this->getName() . " version " . ($version->isDev() ? TextFormat::YELLOW : "") . $version->get(\true) . TextFormat::WHITE . " \"" . $this->getCodename() . "\" (API " . $this->getApiVersion() . ")");
		...
		if($this->getProperty("chunk-generation.use-async", \true)){
	...
	public function dispatchCommand(CommandSender $sender, $commandLine){
		...
		if($sender instanceof Player){
			$message = $this->getAdvancedProperty("messages.unknown-command", "コマンドが見つかりません");
	...
	public function reload(){
		...
		if(($memory = \str_replace("B", "", \strtoupper($this->getConfigString("memory-limit", "256M")))) !== \false){
			...
			$real = ((int) \substr($memory, 0, -1)) * $value[\substr($memory, -1)];
			if($real < 256){
				$this->logger->warning($this->getName() . " may not work right with less than 256MB of RAM", \true, \true, 0);
			}
			@\ini_set("memory_limit", $memory);
		}else{
			$this->setConfigString("memory-limit", "256M");
	...
	public function forceShutdown(){
		...
		try{
			...
			if($this->getProperty("settings.upnp-forwarding", \false) === \true){
			...
			foreach($this->players as $player){
				$player->close(TextFormat::YELLOW . "[ログアウト] " . $player->getName(), $this->getProperty("settings.shutdown-message", "Server closed"));
			}
	...
	public function start(){
		...
		if($this->getProperty("settings.send-usage", \true) !== \false){
...
$version = "3.0.0";
$this->logger->info("§b---------------------------------------------------------------------------");
$this->logger->info("§dサーバー情報");
$this->logger->info("§e|§a ・Minecraft PE\t:§e v0.10.4 alpha");
$this->logger->info("§e|§a ・PocketMine-MP\t:§e BetaBuild #936");
$this->logger->info("§e|§a ・PocketMine-Soft\t:§e 1.4.1(dev-226)");
$this->logger->info("§e|§a ・API version\t\t:§e 1.11.0");
$this->logger->info("§e|§a ・PM-MP派生版\t\t:§e " . $version);
$this->logger->info("§b---------------------------------------------------------------------------");
$this->logger->info("§fPocketMine-MPサーバーを起動しました (起動するのに" . \round(\microtime(\true) - \pocketmine\START_TIME, 3) . "秒かかりました)");
$this->logger->info("§fコマンド一覧を表示するには §a/help§f 又は §a/?§f と入力してください。");
$this->logger->info("§b---------------------------------------------------------------------------");
// スタートメッセージ
	...
	public function checkTicks(){
		if($this->getTicksPerSecond() < 12){
			$this->logger->warning("サーバーに高負荷が発生しています");
		}
	}
	...
	private function titleTick(){
		if(\defined("pocketmine\\DEBUG") and \pocketmine\DEBUG >= 0 and \pocketmine\ANSI === \true){
			echo "\x1b]0;" . "PocketMine-MP派生版" . " | Online " . \count($this->players) . "/" . $this->getMaxPlayers() . " | RAM " . \round((\memory_get_usage() / 1024) / 1024, 2) . "/" . \round((\memory_get_usage(\true) / 1024) / 1024, 2) . " MB | U " . \round($this->mainInterface->getUploadUsage() / 1024, 2) . " D " . \round($this->mainInterface->getDownloadUsage() / 1024, 2) . " kB/s | TPS " . $this->getTicksPerSecond() . " | Load " . $this->getTickUsage() . "%\x07";
		}
	}
