...
class Player extends Human implements CommandSender, InventoryHolder, IPlayer{
	...
	public function spawnTo(Player $player){
		if($this->spawned === \true and $this->dead !== \true and $player->dead !== \true and $player->getLevel() === $this->level and $player->canSee($this)){
	...
	public function __construct(SourceInterface $interface, $clientID, $ip, $port){
		...
		$this->checkMovement = (bool) $this->server->getAdvancedProperty("main.check-movement", \true);
	...
	public function sendChunk($x, $z, $payload){
		...
		$pk->chunkZ = $z;
		$pk->data = $payload;
		$cnt = $this->dataPacket($pk, \true);
		if($cnt === \false or $cnt === \true){
			return;
		}
		$this->chunkACK[$cnt] = \PHP_INT_SIZE === 8 ? ((($x) & 0xFFFFFFFF) << 32) | (( $z) & 0xFFFFFFFF) : ($x) . ":" . ( $z);
	}
	...
	protected function sendNextChunk(){
		...
		if(\count($this->usedChunks) >= 56 and $this->spawned === \false){
			...
			$this->server->getPluginManager()->callEvent($ev = new PlayerJoinEvent($this, TextFormat::YELLOW . "[ログイン] " . $this->getName()));
	...
	protected function orderChunks(){
		...
		for($X = -$side; $X <= $side; ++$X){
			for($Z = -$side; $Z <= $side; ++$Z){
				$chunkX = $X + $centerX;
				$chunkZ = $Z + $centerZ;
				if(!isset($this->usedChunks[$index = \PHP_INT_SIZE === 8 ? ((($chunkX) & 0xFFFFFFFF) << 32) | (( $chunkZ) & 0xFFFFFFFF) : ($chunkX) . ":" . ( $chunkZ)])){
	...
	public function setGamemode($gm){
		...
		if(($this->gamemode & 0x01) === ($gm & 0x01)){
			$this->gamemode = $gm;
			$this->sendMessage("貴方のゲームモードは " . Server::getGamemodeString($this->getGamemode()) . " に変更されました\n");
		}else{
			$this->gamemode = $gm;
			$this->sendMessage("貴方のゲームモードは " . Server::getGamemodeString($this->getGamemode()) . " に変更されました\n");
	...
	protected function getCreativeBlock(Item $item){
		...
		if(!$this->getServer()->getAdvancedProperty("main.filter-creative-items", \true)){
			return 0;
		}
			...
			if($this->isSurvival()){
				if(!$revert and !$this->isSleeping()){
					if($diff > 0.0625 and $this->checkMovement){
						$revert = \true;
						$this->server->getLogger()->warning("[SpeedHack ?] " . $this->getName());
		...
		if($this->spawned){
			...
			if($this->onGround){
				...
				if($this->inAirTicks > 100 and $this->isSurvival() and !$this->isSleeping() and $this->spawned and !$this->server->getAllowFlight()){
					$this->kick("FlyHackの検出をしました(誤作動の可能性もあります)");
					return \false;
				}else{
					++$this->inAirTicks;
				}
		...
		switch($packet->pid()){
			case ProtocolInfo::LOGIN_PACKET:
				...
				if(!$this->server->isWhitelisted(\strtolower($this->getName()))){
					$this->close(TextFormat::YELLOW . "[ログアウト] " . $this->username, "Server is white-listed");

					return;
				}elseif($this->server->getNameBans()->isBanned(\strtolower($this->getName())) or $this->server->getIPBans()->isBanned($this->getAddress())){
					$this->close(TextFormat::YELLOW . "[ログアウト] " . $this->username, "You are banned");
				...
				foreach($this->server->getOnlinePlayers() as $p){
					if($p !== $this and \strtolower($p->getName()) === \strtolower($this->getName())){
						if($p->kick("logged in from another location") === \false){
							$this->close(TextFormat::YELLOW . "[ログアウト] " . $this->getName(), "Logged in from another location");
				...
				if(!($nbt instanceof Compound)){
					$this->close(TextFormat::YELLOW . "[ログアウト] " . $this->username, "Invalid data");
				...
				if($ev->isCancelled()){
					$this->close(TextFormat::YELLOW . "[ログアウト] " . $this->username, $ev->getKickMessage());
				...
				$this->server->getLogger()->info(TextFormat::AQUA . $this->username . TextFormat::WHITE . "(" . $this->ip . ":" . $this->port . ") がサーバーにアクセス中 [ID:" . $this->id . "] (" . $this->level->getName() . " : " . \round($this->x, 4) . " " . \round($this->y, 4) . " " . \round($this->z, 4) . ")");
	...
	public function kick($reason = ""){
		$this->server->getPluginManager()->callEvent($ev = new PlayerKickEvent($this, $reason, TextFormat::YELLOW . "[ログアウト] " . $this->username));
		if(!$ev->isCancelled()){
	...
	public function kill(){
		...
		switch($cause){
			case EntityDamageEvent::CAUSE_ENTITY_ATTACK:
				if($ev instanceof EntityDamageByEntityEvent){
					...
					if($e instanceof Player){
						$message = $this->getName() . " は " . $e->getName() . " に殺された";
						break;
			...
			case EntityDamageEvent::CAUSE_PROJECTILE:
				if($ev instanceof EntityDamageByEntityEvent){
					...
					if($e instanceof Living){
						$message = $this->getName() . " は " . $e->getName() . " に射抜かれた";
						break;
					}
				}
				$message = $this->getName() . " は矢によって射抜かれた";
				break;
			case EntityDamageEvent::CAUSE_SUICIDE:
				$message = $this->getName() . " は自殺した";
				break;
			case EntityDamageEvent::CAUSE_VOID:
				$message = $this->getName() . " は奈落に落ちた";
				break;
			case EntityDamageEvent::CAUSE_FALL:
				if($ev instanceof EntityDamageEvent){
					if($ev->getFinalDamage() > 2){
						$message = $this->getName() . " は落下死した";
						break;
					}
				}
				$message = $this->getName() . " は落下死した";
			...
			case EntityDamageEvent::CAUSE_SUFFOCATION:
				$message = $this->getName() . " は窒息死した";
				break;

			case EntityDamageEvent::CAUSE_LAVA:
				$message = $this->getName() . " は溶岩水泳を試みた";
				break;

			case EntityDamageEvent::CAUSE_FIRE:
				$message = $this->getName() . " はしめやかに爆散した";
				break;

			case EntityDamageEvent::CAUSE_FIRE_TICK:
				$message = $this->getName() . " は燃えカスとなった";
				break;

			case EntityDamageEvent::CAUSE_DROWNING:
				$message = $this->getName() . " は溺死した";
				break;

			case EntityDamageEvent::CAUSE_CONTACT:
				$message = $this->getName() . " は串刺しにされてしまった";
				break;
