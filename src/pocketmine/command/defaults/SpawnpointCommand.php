...
class SpawnpointCommand extends VanillaCommand{
	...
	public function __construct($name){
		parent::__construct(
			$name,
			"プレイヤーのスポーン地点を設定します",
			"/spawnpoint | /spawnpoint プレイヤー | /spawnpoint プレイヤー x y z"
		);
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		if(\count($args) === 4){
			if($level !== \null){
				...
				$sender->sendMessage($target->getDisplayName() . "のスポーン地点を " . $x . " " . $y . " " . $z . " に設定しました");
			...
			if($sender instanceof Player){
				...
				$target->setSpawn($pos);
				$sender->sendMessage($target->getDisplayName() . "のスポーン地点を " . $pos->x . " " . $pos->y . " " . $pos->z . " に設定しました");
