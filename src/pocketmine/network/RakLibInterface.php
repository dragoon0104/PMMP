...
class RakLibInterface implements ServerInstance, SourceInterface{
	...
	public function __construct(Server $server){
		...
		if(!$this->server->getAdvancedProperty("main.session-port-checking", \true)){
	...
	public function closeSession($identifier, $reason){
		if(isset($this->players[$identifier])){
			...
			$player->close(TextFormat::YELLOW . "[ログアウト] " . $player->getName(), $reason);
		}
	}
	...
	private function getPacket($buffer){
		...
		if(($data = $this->getPacketFromPool($pid)) === \null){
